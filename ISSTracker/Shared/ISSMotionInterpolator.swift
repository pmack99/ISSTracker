...
enum ISSMotionInterpolator {
    static func coordinate(at date: Date, current: ISSPosition, previous: ISSPosition?) -> CLLocationCoordinate2D {
        ...
        if let previous, previous.timestamp < current.timestamp {
        ...
        guard age > 0, current.velocity > 0 else { return currentPoint }

        let bearing = motionBearing(current: current, previous: previous)
        let distanceKm = current.velocity / 3600 * age
        return offset(from: currentPoint, bearingDegrees: bearing, distanceKm: distanceKm)
    }

    private static func motionBearing(current: ISSPosition, previous: ISSPosition?) -> Double {
        if let previous {
            let from = CLLocationCoordinate2D(latitude: previous.latitude, longitude: previous.longitude)
            let to = CLLocationCoordinate2D(latitude: current.latitude, longitude: current.longitude)
